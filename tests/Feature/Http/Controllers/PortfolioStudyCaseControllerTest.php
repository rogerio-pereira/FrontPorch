...
it('renders a case study by slug', function () {
    ...
    $this->get('/portfolio/study-case/from-missed-calls-to-booked-jobs')
        ->assertOk()
...
it('falls back to the title and placeholder without gallery images', function () {
    CaseStudy::factory()->create(['title' => 'A quiet launch']);

    $this->get('/portfolio/study-case/a-quiet-launch')
        ->assertOk()
        ->assertInertia(fn (Assert $page) => $page
            ->has('caseStudy', fn (Assert $props) => $props
...
it('returns not found for unknown study cases', function () {
    $this->get('/portfolio/study-case/not-a-real-case-study')->assertNotFound();
});

it('returns not found for soft deleted study cases', function () {
    $caseStudy = CaseStudy::factory()->create(['title' => 'Retired story']);
    $caseStudy->delete();

    $this->get('/portfolio/study-case/retired-story')->assertNotFound();
});
