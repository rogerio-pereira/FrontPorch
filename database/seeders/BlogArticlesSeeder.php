...
{
    /**
     * The author credited on seeded articles.
     */
    protected const AUTHOR = 'Front Porch Creative';
    ...
    /**
     * Create or refresh a single article, keeping its slug and author stable.
     *
     * @param  array{slug: string, title: string, description: string, category: string, content: string, image: string}  $attributes
     */
    protected function persist(array $attributes): void
    {
        $article = BlogArticle::where('slug', $attributes['slug'])->first();
        ...
        if ($article === null) {
            $article = BlogArticle::create($values);
        } else {
            $article->update($values);
        }

        // The BlogArticleObserver derives the slug from the title and credits
        // the authenticated user; seeding runs without a session.
        $forced = [];

        if ($article->slug !== $attributes['slug']) {
            $forced['slug'] = $attributes['slug'];
        }

        if ($article->published_by !== self::AUTHOR) {
            $forced['published_by'] = self::AUTHOR;
        }
        ...
        $article->forceFill($forced)->saveQuietly();
    }

    /**
     * @return list<array{slug: string, title: string, description: string, category: string, content: string, image: string}>
     */
